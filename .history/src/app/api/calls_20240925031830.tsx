...
export async function getRecipe(id: number): Promise<Recipe | null | ReactNode> {
  ...
  try {
    ...
    if (axios.isAxiosError(err)) {
      ...
      return <p>Error getting recipe information: {err.message}</p>;
    } else {
      // Handle any other types of errors (if any)
  ...
  if (response) {
    if (response.status !== 200) {
      ...
      return <p>Error getting recipe information: {response.statusText}.</p>;
    } else {
