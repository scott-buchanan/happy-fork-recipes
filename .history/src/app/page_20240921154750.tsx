      ...
      <div className="gap-5 sm:columns-1 md:columns-3">
        {randopes?.recipes?.map((item: Recipe) => (
          <Link
            ...
            key={item.id}
            className="relative mb-5 block h-auto w-full overflow-hidden rounded-2xl border bg-slate-300"
          >
            <Image
              src={item.image}
              alt={item.title}
            ...
            <div className="absolute bottom-0 w-full p-3 text-right">
              {item.diets
                ...
                .map((item) => (
                  <Pill
                    className="mr-1 inline-block bg-red-800 last:mr-0"
                    key={item}
                    size="sm"
                  >{`${item.slice(0, 1).toUpperCase()}${item.slice(1)}`}</Pill>
                ))}
            </div>
          </Link>
        ))}
