...
export default function RecipeDetails() {
  ...
  const [similarRecipes, setSimilarRecipes] = useState<any | null>(null);
  ...
  useEffect(() => {
    ...
    //   const cachedRecipes: { recipes: SimilarRecipes[] } = JSON.parse(
    //     localStorage.getItem('similarRecipes') || '{ "recipes": [] }',
    //   );

    //   if (cachedRecipes.recipes.length > 0) {
    //     const cachedRecipe: any | undefined = cachedRecipes.recipes.find(
    //       (recipe: Recipe) => recipe.id === id,
    //     );
    //     if (cachedRecipe) {
    //       return cachedRecipe;
      ...
      if (id) {
        ...
        if (response) {
          if (response.status !== 200) {
            ...
            // Load cached recipes from localStorage
            const cachedSimilarRecipes: SimilarRecipes[] = JSON.parse(
              localStorage.getItem('similarRecipes') || '{ "recipes": [] }',
            );
            // Push to array adding new recipe entry
            cachedSimilarRecipes.push({ id: recipeId, similarRecipes: response.data });
            localStorage.setItem('recipeDetails', JSON.stringify(cachedSimilarRecipes));
          }
        }
      }
    };
