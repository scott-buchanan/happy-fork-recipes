        ...
        <>
          <div className="flex items-start gap-16 p-5">
            <article className="mb-3 flex-grow">
              ...
              {/* <div className="gap-5 sm:columns-1 md:columns-2">
                {similarRecipes &&
                  similarRecipes.map((item) => <RecipeCard key={item.id} recipe={item} />)}
              </div> */}
            </article>
                ...
                <ul>
                  {recipeData.extendedIngredients?.map((ingredient) => (
                    <li key={ingredient.id} className="last-of-type:mb-0">
                      {`ID: ${ingredient.id} ${new Fraction(ingredient.amount).toFraction()} ${ingredient.unit} ${capitalize(ingredient.nameClean)}`}
                    </li>
                  ))}
                </ul>
              </div>
